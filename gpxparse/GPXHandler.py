...
class GpxHandler(object):

    def parse_and_remove_fic(self, filename):
        xmlContent = ""
        for line in open(filename).readlines():
            xmlContent += line
        if len(xmlContent) > 0:
            self.parse_and_remove(xmlContent)
            return 0
        else:
            return 1

    def parse_and_remove(self,xmlStringContent): #tiré de la page 186 de Python CookBook Third Edition et de http://lxml.de/tutorial.html
    ...
    def recupere_donnees(self, element):
        retour = {}
        try:
    ...
    def calcule_distance_parcourue(self, donnees_avant,donnees_actuelles):
    ...
    def traduction_gpx_vers_csv(self, xml_str_content):
        points=[]
        for domElem in self.parse_and_remove(xml_str_content):
            points.append(self.recupere_donnees(domElem))
        ...
        for p in points:
            if pAvant is None:
                ...
                avancee = {}
                pEnCours = self.calcule_distance_parcourue(pAvant,p)
